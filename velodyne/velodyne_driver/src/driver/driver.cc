                               ...
                               ros::NodeHandle private_nh)
{
  ...
  hour_ = 0;             // Initialize the hour
  old_packet_counter_ = 0;
  counter2_ = 0;
  counter1_ = 0;
  packet_sub_ = node.subscribe("/packet_counter",400,&velodyne_driver::VelodyneDriver::PacketCallback,this);
  ...
  //make sure there are two elements in the queue
  while(counter2_ == 0)
  {
    while(counter1_ == 0)
    {
      if(!counter_time.empty())
      {
        counter1_time_ = counter_time.front().head.stamp;
        counter1_ = counter_time.front().packet_counter;
        pop_time_counter();
      }
    }
    if(!counter_time.empty())
    {
      counter2_time_ = counter_time.front().head.stamp;
      counter2_ = counter_time.front().packet_counter;
      pop_time_counter();
    }
  }
  ROS_INFO("Counter1 and Counter2 have been parepared");
}


void VelodyneDriver::PacketCallback(const cplus_xsens_driver::packet_counterConstPtr &packet){  
      cplus_xsens_driver::packet_counter packetcounter;
      packetcounter = *packet;
      read_write_mtx_.lock();
      counter_time.push(packetcounter);
      read_write_mtx_.unlock();
}

void VelodyneDriver::pop_time_counter( void )
{
  read_write_mtx_.lock();
  counter_time.pop();
  read_write_mtx_.unlock();
...
bool VelodyneDriver::poll(void)
{
  ...
  if( config_.cut_angle >= 0) //Cut at specific angle feature enabled
  {
    ...
    //TODO(zoeylove)
    //read hours from the NMEA
    while(true)
    {
      while(true)
      {
        
        packet_return res = input_->getPacket(&tmp_packet, config_.time_offset);

        if ( res.rc == 0) 
        {
          packet_counter_ = double(res.timestamp_result)*4/10000.0;
          packet_counter_ += hour_*3600*400;

          //Caculate the hours which aren't contained in the timestamp
          if(packet_counter_ < old_packet_counter_)
          {
            ROS_INFO("HOUR");
            hour_ += 1;
          }
          ...
          if( (packet_counter_>counter1_) && (packet_counter_<counter2_) )
          {
              synctime_nsec_ = floor((
                (packet_counter_ - counter1_) * counter2_time_.toNSec() + 
                (counter2_ - packet_counter_) * counter1_time_.toNSec())
                /(counter2_-counter1_));
              time2_.sec = floor(synctime_nsec_ / (1e9));
              time2_.nsec = (synctime_nsec_ % 1000000000 );
              find_flag = true;
          }

          if ((packet_counter_ - counter2_) >0)
          {
              synctime_nsec_ = floor((
                (packet_counter_ - counter1_) * counter2_time_.toNSec() + 
                (counter2_ - packet_counter_) * counter1_time_.toNSec())
              ...
              time2_.sec = floor(synctime_nsec_/(1e9));
              time2_.nsec = (synctime_nsec_ % 1000000000 );
              while((counter_time.size()>1) && ((packet_counter_ - counter2_)>0) ){
                pop_time_counter();
                counter1_time_ = counter2_time_;
                counter1_ = counter2_;
                counter2_time_ = counter_time.front().head.stamp;
                counter2_ = counter_time.front().packet_counter;
            }
          ...
          {
            ROS_ERROR("ERROR");
            ROS_INFO("counter1_ is %u ,counter2_ is %u ,packet_counter is %f",counter1_,counter2_,packet_counter_);
          }
          old_packet_counter_ = packet_counter_;
          tmp_packet.stamp = time2_;
          break;       // got a full packet
        }
        if (res.rc < 0) return false; // something is wrong
  ...
  // reading and publishing scans as fast as possible.
    ...
    for (int i = 0; i < config_.npackets; ++i)
    {
      while (true)
        ...
        packet_return res = input_->getPacket(&scan->packets[i], config_.time_offset);
