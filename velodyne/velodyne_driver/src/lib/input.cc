...
namespace velodyne_driver
{
  ...
  packet_return InputSocket::getPacket(velodyne_msgs::VelodynePacket *pkt, const double time_offset)
  {
    ...
    uint32_t timestamp = pkt->data[1203]*pow(2,24) + pkt->data[1202]*pow(2,16) + pkt->data[1201]*pow(2,8) + pkt->data[1200];

    pkt_re.rc = 0;
    pkt_re.timestamp_result = timestamp; 
    return pkt_re;
  }
  ...
  /** @brief Get one velodyne packet. */
  packet_return InputPCAP::getPacket(velodyne_msgs::VelodynePacket *pkt, const double time_offset)
  {
